/* ═══════════════════════════════════════════════
   BAFANA KIT — HOME PRODUCT PAGE  (v7)
   ═══════════════════════════════════════════════ */
...
// ─── Resolve home jersey variant + product data ───
...
async function _getProduct() {
  ...
  for (const { node } of json?.data?.products?.edges || []) {
    if (!node.handle.toLowerCase().includes('away')) {
      _productCache = {
        ...
        title:     node.title || 'Bafana Bafana Home Jersey 2026',
        image:     node.featuredImage?.url || './images/home-jersey-front.png',
      };
  ...
  _productCache = {
    variantId: null,
    title: 'Bafana Bafana Home Jersey 2026',
